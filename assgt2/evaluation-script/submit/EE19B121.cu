...
__global__ void abcmult(int *dA, int *dB, int *dC, int k, int bsize, int *dtemp) {
	int multblockrow = blockIdx.x;
	int multblockcol = blockIdx.y;
	
	int localrow = threadIdx.y;
	int localcol = threadIdx.x;
	
	int rowid = multblockrow * bsize + localrow;
	int colid = multblockcol * bsize + localcol;
