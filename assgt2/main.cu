...
__global__ void addmats(int *A, int *B, int *sumres, int rowmax, int colmax) {
	...
	if ((rowidA<colmax) && (colidA<rowmax)) {	
		...
		//printf("addmats1: (%d, %d):(%d, %d) - %d\n", blockIdx.y, blockIdx.x, threadIdx.y, threadIdx.x, A[tidA]);
	}
	__syncthreads();
	...
	if ((rowid<rowmax) && (colid<colmax))	{
		...
		//printf("addmats2: (%d, %d):(%d, %d) - %d\n", blockIdx.y, blockIdx.x, threadIdx.y, threadIdx.x, sumres[tid]);
	}
}

__global__ void displ(int *A, int r, int c){
	for(int i=0; i<r; i++) {
		for(int j=0; j<c; j++)
			printf("%d ", A[i*c+j]);
		printf("\n");
...
void addmats_temp(int *A, int *B, int *sumres, int rm, int cm) {
	
	for(int i=0; i<rm; i++) {
		for(int j=0; j<cm; j++) {
			sumres[i*cm+j] = A[i+j*rm]+B[i*cm+j];
			printf("%d ", sumres[i*cm+j]);
		}
		printf("\n");
	}
}
...
__global__ void badmult(int *A, int *B, int *multres, int a, int b, int c) {
	...
	for(int blockiter=0; blockiter<2+(b/32); blockiter++) {
		...
		for (int i=0; i<16 ;i++) {
			...
			if ((blockIdx.y*16 + threadIdx.y < a)  && (blockiter*16 + i < b) && (blockIdx.x*16 + threadIdx.x < c)) {
				...
				//printf("badmult3:%d (%d, %d):(%d, %d) - %d\n", i, blockIdx.y, blockIdx.x, threadIdx.y, threadIdx.x, multres[tid]);
			}
		}
...
/*__global__ void badmult(int * A, int *B, int *X, int rowA, int cols, int rowB){
	__shared__ int tempRowA[32][32];
	__shared__ int tempRowB[32][32];

	int j = blockIdx.x*32 + threadIdx.x;
	int i = blockIdx.y*32 + threadIdx.y;
	int iTemp, jTemp;
	for(int k=0; k<cols; k+=32){
		if((k+threadIdx.x)<cols){
			if(i<rowA){
				tempRowA[threadIdx.y][threadIdx.x] = A[i*cols + (k+threadIdx.x)]; 
			}
			jTemp = blockIdx.x*32 + threadIdx.y;
			if(j<rowB){
				tempRowB[threadIdx.y][threadIdx.x] = B[jTemp*cols + (k+threadIdx.x)]; 
			}
		}
		__syncthreads();
		...
		if(i<rowA && j<rowB){
			int temp=0;
			for(int l=0; (l<32 && (k+l)<cols); l++){
				temp+=tempRowA[threadIdx.y][l]*tempRowB[threadIdx.x][l];
			}
			X[i*rowB + j] += temp;
		}
	}
...
// A = aXb and B = cXb
void badmult_temp(int *A, int *B, int *res, int a, int b, int c) {
	int temp=0;
	for(int i=0; i<a; i++) {
		for(int j=0; j<c; j++) {
			temp = 0;
			for(int k=0; k<b; k++) {
				temp += A[i*b+k]*B[j*b+k];
			}
			res[i*c+j] = temp;
			printf("%d ", temp);
		}
...
__global__ void goodmult(int *A, int *B, int *multres, int a, int b, int c) {	
	...
	if ((colidA<b) && (colidB<c)) {
		for(int blockiter=0; blockiter<1+(b/32); blockiter++) {
			immtemp = 0;
			...
			for(int i=0; i<32; i++) {
				...
				if ((rowidA<a) && (rowidB<a)) {
					...
					//printf("goodmult1: (%d, %d):(%d, %d) - %d, %d\n", blockIdx.y, blockIdx.x, threadIdx.y, threadIdx.x, A[tidA], B[tidB]);
				}
			}
		...
		//printf("goodmult2: (%d, %d):(%d, %d) - %d\n", blockIdx.y, blockIdx.x, threadIdx.y, threadIdx.x, multres[tid]);
	}
}

//A = aXb  B = aXc
void goodmult_temp(int *A, int *B, int *res, int a, int b, int c) {
	int temp=0;
	for(int i=0; i<b; i++) {
		for(int j=0; j<c; j++) {
		temp=0;
			for(int k=0; k<a; k++) {
				temp += A[k*b+i]*B[k*c+j];
			}
			res[i*c+j] = temp;
			printf("%d ", temp);
		}
...
void compute(int p, int q, int r, int s, int *h_matrixA, int *h_matrixB, 
	...
	printf("\nA---------------------\n");
	displ<<<1,1>>>(d_matrixA, p, q);
	cudaDeviceSynchronize();
	printf("\nB---------------------\n");
	displ<<<1,1>>>(d_matrixB, q, p);
	cudaDeviceSynchronize();
	printf("\nAtrans+B---------------------\n");
	...
	displ<<<1, 1>>>(addres, q, p);
	cudaDeviceSynchronize();
	printf("\n---------------------\n");
	int *temp;
	temp = (int*) malloc(p * q * sizeof(int));
	addmats_temp(h_matrixA, h_matrixB, temp, q, p);
	
	printf("\nC*Dtrans---------------------\n");
	//2. Computing C*Dtrans
	dim3 badmult_dims(2+(q/16), 2+(s/16), 1);
	...
	displ<<<1, 1>>>(multres, q, s);
	cudaDeviceSynchronize();
	printf("\n---------------------\n");
	int *temp1;
	temp1 = (int*) malloc(s * q * sizeof(int));
	badmult_temp(h_matrixC, h_matrixD, temp1, q, r, s);
	...
	displ<<<1, 1>>>(d_matrixX, p, s);
	cudaDeviceSynchronize();
	printf("\n---------------------\n");
	int *temp2;
	temp2 = (int*) malloc(p * s * sizeof(int));
	goodmult_temp(temp, temp1, temp2, q, p, s);
	exit(0);
